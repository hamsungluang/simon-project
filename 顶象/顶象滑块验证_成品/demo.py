...
class DX_hk(object):
    ...
    def get_api(self):
        ...
        api_res = self.session.get("https://cap.dingxiang-inc.com/api/a", headers=self.headers, params=params,
                                   verify=False).json()
        print(api_res)
        # 获取
        ...
        url_name = re.search(r'https://static.dingxiang-inc.com/picture/dx/.*?/.*?/(.*?).webp',
                             image_item['bg_url']).group(1)
